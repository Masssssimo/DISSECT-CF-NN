...
public class NerualNetworkConsolidator extends ModelBasedConsolidator {
    ...
    public InfrastructureModel optimize(final InfrastructureModel sol){
        ...
        for(int i=0;i<sol.items.length;i++){
            ...
            data.put(0,1,sol.items[i].basedetails.vm.getResourceAllocation().allocated.getRequiredCPUs());
            data.put(0,2,sol.items[i].basedetails.vm.getResourceAllocation().allocated.getRequiredMemory());
            data.put(0,3,sol.items[i].getHostID());
        ...
        for(int i=0;i<sol.items.length;i++){
            ...
            // Add all VM's hosts (Continuing from index position x)
            int x = 3;
            for (int j = 0; j < sol.items.length; j++) {
                data.put(0,x,sol.items[j].getHostID());
                x++;
            ...
            // Adding all PM's hosting amount
            for (int k = 0; k < sol.bins.length; k++) {
                data.put(0,x,sol.bins[k].getVMs().size());
                x++;
            }
            ...
            System.out.println("VM :- "+sol.items[i].getHostID()+ "\t-->\tPM :- "+idx);
